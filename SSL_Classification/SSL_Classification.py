...
def pseudo_label(x_lbl,y_lbl,x_unlbl,k,*arg):
    y_lbl=y_lbl.reshape(-1,1)
	    ...
	    for i in range(n2):
	        ...
	        y_test[i]=temp3[0]
	    return y_test
    #Step 1: train with labelled data and generate pseudo labels
    y_unlbl=KNN(x_lbl,y_lbl,x_unlbl,k)
    y_unlbl=y_unlbl.reshape(-1,1)
    #Step 2:append pseudolabels
    x_lbl=np.vstack((x_lbl,x_unlbl))
    y_lbl=np.vstack((y_lbl,y_unlbl))
